...
//		request.setAttribute("list", mapList);
	...
	@GetMapping("getName.do")                                  // 데이터 담아주려고 model객체 선언하기
	public String getNameGet(@RequestParam("nameNo") int nameNo, Model model) {
	...
	public String deleteName(@RequestParam("nameNo") int nameNo, Model model) {
		// 1. nameNo에 해당하는 사용자 삭제
		homeService.deleteName(nameNo);
		
		// 2. 삭제된 사용자를 제외한 사용자 목록 조회
		List<NameVO> nameList = homeService.getNameList();
		
		// 3. 재조화된 사용자 목록을 화면으로 전달
		model.addAttribute("nameList", nameList);
		return "getNameList";
	}
	...
	public String updateName(@RequestParam Map<String, Object> paramMap) {
		...
		// redirect vs forward
		// redirect: WAS단에서 실행되서 주소창의 url도 변경
		// forward: client(브라우저)단에서 실해되서 주소창의 url이 변경되지 안흥
