...
<body>
	...
	<script>
		$(function(){ // $(document).ready() 대신사용
			$(".pagination a").on("click", function(e) {
				e.preventDefault(); // a태그 동작 막아줌
				
				$("input[name='pageNum']").val($(this).attr("href"));
				$("#searchForm").submit();
			});
		
			$("#btnSearch").on("click", function(){ // 검색했을 때
				$("input[name='pageNum']").val(1);  // 페이지 넘버는 1 페이지로 보여지고
				$("#searchForm").submit();          // 검색한 내용 보여주기
			});
		});
